...
use App\Controllers\CursoIncritosController;
use Dotenv\Dotenv;
...
// Controlador
$cursoController = new CursoIncritosController();

// Rutas REST
if (preg_match('#^cursos_inscritos.php/(\d+)$#', $relativeUri, $matches)) {
    $id = $matches[1];
    switch ($method) {
        case 'GET':
...
} elseif ($relativeUri === '/cursos_inscritos.php' && $method === 'GET') {
    $cursoController->getAll();
} elseif ($relativeUri === '/cursos_inscritos.php' && $method === 'POST') {
